...
5월 4일~6월 1일까지의 가상 데이터(알림장·주간리포트·공지사항·일정표·식단표·보호자문의)를 일괄 적재하는 스크립트입니다.
...
> **주의**: 실행 시 `report`, `notice`, `inquiry`, `meal_log`, `schedule_event`, `program`, `notice_board` 테이블의 기존 데이터가 모두 삭제된 후 새 시드가 들어갑니다. 회원(`app_user`), 어르신(`resident`), 보호자(`guardian`), 시설(`facility`), 담당(`assignment`) 테이블은 유지됩니다.

---

## 🌐 외부 접속 (ngrok)

발표장, 카페, 스마트폰 LTE 등 **내 컴퓨터와 다른 네트워크**에서 케어알림장에 접속하려면 ngrok을 사용합니다.

### 사전 준비

1. [ngrok.com](https://ngrok.com/) 에서 무료 회원가입
2. Windows용 ngrok 다운로드 후 압축 해제
3. 대시보드에 표시된 인증 토큰 등록 (최초 1회)
   ```bash
   ngrok config add-authtoken [내_인증_토큰]
   ```

### 실행 순서

무료 플랜은 터널 1개만 사용할 수 있으므로, **Vite 프록시**를 활용해 프론트엔드 포트(5173) 하나만 외부에 열면 백엔드 API까지 자동으로 중계됩니다.

**Step 1 — 백엔드 서버 켜기** (포트 8000)
...
**Step 2 — 프론트 서버 켜기** (포트 5173, 외부 접속 허용)
...
**Step 3 — ngrok 실행** (새 터미널 창)

```bash
ngrok http 5173
```

화면에 나오는 `Forwarding` 주소를 확인합니다:

```
Forwarding    https://xxxxx.ngrok-free.dev -> http://localhost:5173
```

**Step 4 — vite.config.ts에 ngrok 호스트 등록**

`frontend/vite.config.ts`의 `allowedHosts`에 발급받은 주소의 도메인을 추가합니다:
...
export default defineConfig({
  ...
  server: {
    port: 5173,
    allowedHosts: ['xxxxx.ngrok-free.dev'],  // ← ngrok 도메인으로 교체
    proxy: {
      '/api': {
...
프론트 서버를 재시작(`Ctrl+C` 후 `npm run dev -- --host`)한 뒤, 스마트폰 브라우저에서 발급받은 `https://xxxxx.ngrok-free.dev` 주소로 접속하면 케어알림장 화면이 표시됩니다.

> **참고**: ngrok 무료 플랜은 접속 시 "Visit Site" 확인 버튼이 한 번 나타납니다. 눌러주시면 정상 접속됩니다.

### 되돌리기 (로컬 개발로 복구)

ngrok 사용이 끝나면 `vite.config.ts`를 원래대로 복구합니다:

```bash
# 백업해 둔 원본으로 복구
copy frontend\vite.config_original.ts frontend\vite.config.ts
```
...
### 방법 1 — Docker Desktop에서 마우스로 확인 (가장 쉬움)

1. Docker Desktop 실행 → **Containers** → `care-postgres` 클릭
2. 상단 **Terminal** 탭 클릭
3. 아래 명령어 입력:
   ```bash
   psql -U postgres -d care_notice
   ```
4. 데이터 조회:
   ```sql
   -- 직원/보호자 명단
   ...
   -- 어르신 명단
   SELECT id, name, room_number, care_level, precautions FROM resident;
...
### 방법 2 — PowerShell에서 한 줄로 조회

```powershell
# 직원/보호자 명단
docker exec -it care-postgres psql -U postgres -d care_notice -c "SELECT id, email, name, role FROM app_user;"

# 어르신 명단
docker exec -it care-postgres psql -U postgres -d care_notice -c "SELECT id, name, room_number, care_level FROM resident;"

# 테이블별 데이터 건수 확인
docker exec -it care-postgres psql -U postgres -d care_notice -c "SELECT 'program' as t, count(*) from program UNION SELECT 'notice', count(*) from notice UNION SELECT 'report', count(*) from report UNION SELECT 'notice_board', count(*) from notice_board UNION SELECT 'schedule_event', count(*) from schedule_event UNION SELECT 'meal_log', count(*) from meal_log UNION SELECT 'inquiry', count(*) from inquiry;"
```

### 방법 3 — GUI 프로그램 (DBeaver)

[DBeaver](https://dbeaver.io/download/)를 설치한 뒤 새 연결 추가:

| 항목 | 값 |
|---|---|
| Host | `localhost` |
| Port | `5432` |
| Database | `care_notice` |
| Username | `postgres` |
| Password | `devpass` |
