...
const CARDS: CardDef[] = [
  ...
  { key: 'album',     label: '앨범',      icon: '📷', color: 'bg-pink-100',   borderColor: 'border-pink-200',   path: '/albums' },
...
function statusText(key: string, summary: HomeSummary): string {
  switch (key) {
    ...
    case 'inquiry': {
      const parts: string[] = []
      if (summary.inquiry.unreadCount > 0)    parts.push(`미확인 ${summary.inquiry.unreadCount}건`)
      if (summary.inquiry.needsAnswerCount > 0) parts.push(`답변 대기 ${summary.inquiry.needsAnswerCount}건`)
      return parts.length > 0 ? parts.join(' · ') : '새 문의 없음'
    }
    case 'menu':
    ...
    case 'album':
      return `이번 달 ${summary.albums?.thisMonthCount ?? 0}건`
    default:
      return '준비 중'
  }
}
...
function hasDot(key: string, summary: HomeSummary): boolean {
  ...
  if (key === 'board')
    return summary.board.unreadCount > 0
  if (key === 'inquiry')
    return summary.inquiry.unreadCount > 0 || summary.inquiry.needsAnswerCount > 0
  if (key === 'menu')
